...
const Def* LowerZip::rewrite(const Def* def) {
    ...
    if (auto zip = match<core::zip>(def)) {
        ...
        { // construct body
            ...
            auto idx     = core::op(core::conv::u2u, w.type_int(arr_len), iter);
        ...
        auto new_zip =
            w.app(direct::op_cps2ds(wrapper), w.tuple({w.lit_int(iter_type, 0_s), w.op_bitcast(iter_type, arr_len),
                                                       w.lit_int(iter_type, 1), w.tuple(acc_args), for_body}));
